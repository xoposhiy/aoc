...
for line in inp.splitlines():
    ...
    letter = parts[1][0]
    pwd = parts[2]
    (mi, ma) = map(int, parts[0].split("-"))
    actualCount = len(re.findall(letter, pwd))
    if actualCount >= mi and actualCount <= ma:
        good+=1
    if (pwd[mi-1] == letter or pwd[ma-1] == letter)and(pwd[mi-1] != letter or pwd[ma-1] != letter) :
        good2+=1
print(good)
print(good2)