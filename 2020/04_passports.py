...
inp = """eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
...
validInp = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
fields = "byr:(\d+)\\b iyr:(\d+)\\b eyr:(\d+)\\b hgt:(\d+)(cm|in)\\b hcl:\#[0-9a-fA-F]{6}\\b ecl:(amb|blu|brn|gry|grn|hzl|oth)\\b pid:\d{9}\\b".split();

def validfield(doc, pattern):
    match = re.search(pattern, doc)
    
    if match == None:
        return False
    if any(pattern.startswith(x) for x in "byr iyr eyr".split()):
        v = int(match.group(1))
        if pattern.startswith("byr") and (v < 1920 or v > 2002):
            return False;               
        if pattern.startswith("iyr") and (v < 2010 or v > 2020):
            return False;               
        if pattern.startswith("eyr") and (v < 2020 or v > 2030):
            return False;
            
    if pattern.startswith("hgt"):
        v = int(match.group(1));
        if match.group(2) == "cm" and (v < 150 or v > 193):
            return False;
        if match.group(2) == "in" and (v < 59 or v > 76):
            return False;
    return True;
    
def valid1(doc):
    return all(doc.find(f[0:4]) >= 0 for f in fields)
    
def valid2(doc):
    return all(validfield(doc, f) for f in fields)

docs = inp.split("\n\n")
valids1 = sum(valid1(doc) for doc in docs);
print("Part One", valids1)

valids2 = sum(valid2(doc) for doc in docs);
print("Part Two", valids2)
