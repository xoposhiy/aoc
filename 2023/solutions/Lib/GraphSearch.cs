...
public static class GraphSearch
{
    ...
    public static List<(TNode node, int index)> GetCycle<TNode>(
        this IEnumerable<TNode> sequence) where TNode : notnull
    {
        if (sequence == null) throw new ArgumentNullException(nameof(sequence));
        var period = new List<(TNode node, int index)>();
        var seenAt = new Dictionary<TNode, int>();
        var index = 0;
        var inCycle = false;
        TNode? cycleStart = default; 
        foreach (var node in sequence)
        {
            if (!inCycle)
            {
                if (seenAt.TryGetValue(node, out var prevIndex))
                {
                    cycleStart = node;
                ...
                {
                    seenAt.Add(node, index);
                }
            }
            else
            {
                if (EqualityComparer<TNode>.Default.Equals(node, cycleStart))
                    break;
    ...
    public static (int start, int period) GetCycleFast<TNode>(TNode start, Func<TNode, TNode> next)
        where TNode : notnull
    {
        return start.MakeSequence(next).GetCycleFast();
    }

    public static (int start, int period) GetCycleFast<TNode>(
        this IEnumerable<TNode> sequence) where TNode : notnull
    {
        if (sequence == null) throw new ArgumentNullException(nameof(sequence));
        var seenAt = new Dictionary<TNode, int>();
