...
def parseWirePart(p):
    return (dirs[p[0]], int(p[1:]))

def parseWire(s):
    return list(map(parseWirePart, s.split(",")))
...
def makeImage(w1, w2):
    minX = min(w1+w2, key=lambda x:x[0])[0]
    maxX = max(w1+w2, key=lambda x:x[0])[0]
    minY = min(w1+w2, key=lambda x:x[1])[1]
    maxY = max(w1+w2, key=lambda x:x[1])[1]
    w = maxX-minX+1
    h = maxY-minY+1

    scale = 10
    img = Image.new('RGB', (w//scale+1, h//scale+1), color = 'black')
    ps = img.load()
    for p in w1:
        ps[(p[0]-minX)//scale, (p[1]-minY)//scale] = (255,255,0)

    for p in w2:
        ps[(p[0]-minX)//scale, (p[1]-minY)//scale] = (255,0, 255)

    ps[(-minX)//scale, (-minY)//scale] = (255,255, 255)

    img.save('d03_wires.png')
...
wires = list(map(parseWire, input.split("\n")))
...
intersection = min(filter(lambda p: p in w1d, w2), key=manhatToZero)
print("Part One: ", intersection)
