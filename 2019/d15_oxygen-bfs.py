...
def show(m, droid):
    minX = min([p[0] for p in m.keys()])
    maxX = max([p[0] for p in m.keys()])
    minY = min([p[1] for p in m.keys()])
    maxY = max([p[1] for p in m.keys()])
    res = ''
...
def move (pos, action):
    return (pos[0] + dirs[action][0], pos[1] + dirs[action][1])

def get_path(f, target):
    path = []
    while target != None:
        source = f[target]
        if source[0] != None:
            path.append((source[0], target))
        target = source[1]
    path.reverse()
    return path

def find_path(m, start, is_target):
    q = Queue()
    q.put_nowait(start)
    f = {start: (None, None, 0)}
    maxP = 0
    while not q.empty():
        ...
        for a in actions:
            t = move(p, a)
            if not t in f and ((not t in m) or (m[t] != '#')):
                q.put_nowait(t)
                maxP = max(maxP, f[p][2]+1)
                f[t] = (a, p, maxP)
    return None
...
while True:
    if len(path) == 0:
        path = find_path(m, pos, lambda p: not p in m)
    if path == None:
        break
    action = path.pop(0)[0]
    t = move(pos, action)
...
path = find_path(m, (0, 0), lambda p: p in m and m[p] == 'X')
show(m, (0, 0))
print("Part One: ", len(path))
oxigen = path[len(path)-1][1]

print("Part Two: ", bfs(m, oxigen))